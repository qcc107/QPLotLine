...
@implementation QPlotLine

- (void)dealloc
{
    [_xTickMarks release];
    [_yTickMarks release];
    [_xTickLabels release];
    [_yTickLabels release];
    [_title release];
    [super dealloc];
}
...
-(void)drawPlot {
    ...
    CGFloat xAxisLabelPadding = 3.0; //Padding on both sides of the label
    ...
    if (self.yTickLabels == nil) {
        ...
        labelWidth = [[self.yTickLabels lastObject] sizeWithFont:font].width;
    }
    
    CGFloat yAxisLabelPadding = 5.0;
    CGFloat yAxisWidth = labelWidth + yAxisLabelPadding*2;
    
    CGFloat plotRightSidePadding = 12.0; //Padding on the right of the graph, its nice to add some so the x labels dont get chopped off early.
    ...
    [[self.dataSource dataForLinePlot:self] enumerateObjectsUsingBlock:^(NSDictionary * obj, NSUInteger idx, BOOL *stop) {
        ...
        NSArray *xData = [obj objectForKey:kXaxisData];
        NSArray *yData = [obj objectForKey:kYaxisData];
        if ([obj objectForKey:kLineColor]) {
            [[obj objectForKey:kLineColor] setStroke];
        }else
        {
        ...
        [plotPath setLineJoinStyle:kCGLineJoinMiter];
        [plotPath setLineCapStyle:kCGLineCapSquare];
        ...
        CGFloat blue;
        CGFloat alpha;
        [[obj objectForKey:kLineColor] getRed:&red green:&green blue:&blue alpha:&alpha];
        
        CGFloat colors[8] = {red, green, blue, 0.3,
            red, green, blue, 0.0};
        CGFloat colorStops[2] = {0.0, 0.5};
        CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
        CGGradientRef fillGradient = CGGradientCreateWithColorComponents(colorSpace, colors, colorStops, 2);
        CGColorSpaceRelease(colorSpace);
        
        CGContextDrawLinearGradient(con, fillGradient, gradStartPoint, gradEndPoint, 0);
        CGContextRestoreGState(con);
    }]; // enum all lines data
